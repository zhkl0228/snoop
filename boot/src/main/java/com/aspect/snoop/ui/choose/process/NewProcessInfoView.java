...
public class NewProcessInfoView extends javax.swing.JDialog {
    ...
    public NewProcessInfoView(java.awt.Frame parent, boolean modal) {
        ...
        if (lastStartSession != null) {
            if (lastStartSession.getMainClass() != null) {
                jLabelMainClass.setText(lastStartSession.getMainClass());
            }
            if (lastStartSession.getArguments() != null) {
                txtArguments.setText(lastStartSession.getArguments());
            }
            if (lastStartSession.getJavaArguments() != null) {
                txtJvmArguments.setText(lastStartSession.getJavaArguments());
            }
            if (lastStartSession.getWorkingDir() != null) {
                txtWorkingDir.setText(lastStartSession.getWorkingDir());
            }
            ...
            txtGuiDelay.setText(Integer.toString(lastStartSession.getGuiDelay()));
        }
    }
    ...
    public void addClasspathEntry() {
        ...
        for (File selectedFile : selectedFiles) {
            ...
            if ( selectedFile.isFile()) {
                try {
                    ClassPath entry = new SmartURLClassPath(selectedFile.toURI().toURL());
                    cp.appendClassPath(entry);
                ...
                File[] files = selectedFile.listFiles(jarFilter);
                if (files != null) {
                    for(File file : files) {
                        try {
                            ClassPath entry = new SmartURLClassPath(file.toURI().toURL());
    ...
    private static SnoopSession lastStartSession;
    ...
    public void startProcess() {
        ...
        model.setClasspathString(Util.convertListToString(classpath.getEntries(), File.pathSeparator));
        try {

        ...
        lstJavaProcesses.addListSelectionListener( new ListSelectionListener() {
            ...
            public void valueChanged(ListSelectionEvent e) {
                if ( ! lstJavaProcesses.isSelectionEmpty() ) {
                    ...
                    lblPid.setText(String.valueOf(desc.getId()));
    ...
    private void loadProcesses() {
        ...
        try {
            ...
            for (Integer jvmid: jvmids) {
                ...
                output.append(" ");
                final String longPathsMainClass = MonitoredVmUtil.mainClass(vm, JPS_SHOW_LONG_PATHS);
                output.append(longPathsMainClass);

                final String jvmArgs = MonitoredVmUtil.jvmArgs(vm);
                ...
                JVMDescriptor desc = new JVMDescriptor(jvmid, mainClassName, isAttachable);
                desc.setJVMArguments(jvmArgs);
                final String mainArgs = MonitoredVmUtil.mainArgs(vm);
                desc.setMainArguments(mainArgs);
                final String commandLine = MonitoredVmUtil.commandLine(vm);
                desc.setCommandLine(commandLine);
                final String vmVersion = MonitoredVmUtil.vmVersion(vm);
                desc.setVMVersion(vmVersion);
                
                final String mainClass = MonitoredVmUtil.mainClass(vm, false);

...
public class Hook2JavaUtil {
    ...
    public static MethodChanges hook2Java(FunctionHook hook, InstrumentationManager manager) {
        ...
        if (hook.shouldPrintParameters()) {
            javaCode.append("  com.aspect.snoop.agent.SnoopAgent.getMainView().printParameters(\"").append(hook.getClazz().getName()).append("\", ").append(id).append(", $args, $sig);");
            javaCode.append(nl);
        }

        if (hook.shouldPrintStackTrace()) {
            javaCode.append("  com.aspect.snoop.agent.SnoopAgent.getMainView().printStackTrace(\"").append(hook.getClazz().getName()).append("\", ").append(id).append(", $args, $sig);");
            javaCode.append(nl);
        }

        if (hook.shouldPause()) {
            javaCode.append("  com.aspect.snoop.agent.SnoopAgent.getMainView().pause(\"").append(hook.getClazz().getName()).append("\", ").append(id).append(", $args, $sig);");
            javaCode.append(nl);
        }
        ...
        if (hook.shouldTamperParameters()) {
            ...
            try {
                ...
                javaCode.append("  mods = com.aspect.snoop.agent.SnoopAgent.getMainView().tamperWithParameters(\"").append(hook.getClazz().getName()).append("\", ").append(id).append(", $args, $sig);");
                javaCode.append(nl);
        ...
        changes.initialize(vars.toArray(new LocalVariable[0]), javaCode.toString(), "");
